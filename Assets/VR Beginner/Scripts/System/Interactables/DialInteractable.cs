...
{
    public float targetAngle = 90f; // Specify the target angle for each object individually
    ...
    void Start()
    {
        ...
        if (RotatingRigidbody == null)
        {
            ...
            if (RotatingRigidbody == null)
            {
                Debug.LogError("No Rigidbody found on the DialInteractable or its children.");
            }
        }
        
        if (GetComponent<Collider>() == null)
        {
            Debug.LogError("No Collider component found on the DialInteractable GameObject.");
        }
    ...
    protected override void OnSelectEntered(SelectEnterEventArgs args)
    {
        ...
        // Rotate the dial immediately upon grabbing
        RotateDial();
    }

    void RotateDial()
    {
        // Determine the rotation amount based on the target angle
    ...
    void RotateDial(float angle)
    {
        // Get the world rotation axis
        Vector3 worldRotationAxis = m_SyncTransform.TransformDirection(LocalRotationAxis);

        // Rotate the dial by the specified angle
        Quaternion newRot = Quaternion.AngleAxis(angle, worldRotationAxis) * m_SyncTransform.rotation;
        ...
        // Invoke events
        OnDialAngleChanged.Invoke(m_CurrentAngle);
        OnDialChanged.Invoke(this);
    }
    
    private void RotateDialToTargetAngle()
    {
        // Calculate the target rotation
        Vector3 worldRotationAxis = transform.TransformDirection(LocalRotationAxis);
        Quaternion targetRotation = Quaternion.AngleAxis(targetAngle, worldRotationAxis) * transform.rotation;
        ...
        OnDialAngleChanged.Invoke(targetAngle);
        OnDialChanged.Invoke(this);
    }
    ...
    public override bool IsSelectableBy(XRBaseInteractor interactor)
    {
        ...
        bool isSelectable = base.IsSelectableBy(interactor) && (interactionLayerMask.value & interactorLayerMask) != 0;
        if (!isSelectable)
        {
            Debug.LogWarning($"Interactor layer mask does not match: {interactor.gameObject.name}");
        }
        return isSelectable;
    }
